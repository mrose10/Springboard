...
/* In MySQL */
SELECT  f.memid,concat(firstname, ' ',surname) AS member_name,
FROM Members
...
/*In MySQL:*/
SELECT f.name AS Facility_Name, MONTH(starttime) AS Month, SUM(slots) AS Member_Bookings 
FROM `Bookings` 
INNER JOIN `Facilities` as f
USING (facid)
WHERE memid <> '0' 
...
/*In SQLITE:*/
SELECT f.name AS Facility_Name,strftime("%m", starttime) as Month , SUM(slots) AS Member_Bookings 
FROM Bookings
INNER JOIN Facilities as f
USING (facid)
WHERE memid <> '0' 
